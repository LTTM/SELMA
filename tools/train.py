...
class Trainer():
    def __init__(self, args, writer, logger):
        ...
        self.tset = args.dataset(root_path=args.root_path,
                                 ...
                                 crop_to=args.crop_size if args.crop_images,
    ...
    def train(self):
        ...
        for epoch in range(epochs):
            ...
            self.logger.info("Epoch done, validating")
    ...
    def train_epoch(self, epoch):
        pbar = tqdm(self.tloader, total=self.args.self.validate_every_steps, desc="Training Epoch %d"%epoch)
        metrics = Metrics(self.tset.cnames)
        for i, sample in enumerate(pbar):
            
            curr_iter = self.validate_every_steps*epoch + i
            ...
            pred = torch.argmax(out[0].detach(), dim=0)
            metrics.add_sample(pred, y)
            self.writer.add_scalar('step_mIoU', metrics.percent_mIoU(), curr_iter)
            
            self.optim.step()
            
            if curr_iter >= self.args.iterations:
                break
        
        self.writer.add_image("train_input", tset.to_rgb(x[0]), epoch, dataformats='HWC')
        self.writer.add_image("train_label", tset.color_labels(y[0]), epoch, dataformats='HWC')
        self.writer.add_image("train_pred", tset.color_labels(pred), epoch, dataformats='HWC')
        
        miou = metrics.percent_mIoU()
        self.writer.add_scalar('train_mIoU', miou, epoch)
        self.logger.info("Epoch %d score: %.2f"%(epoch, miou))
        
    def validate(self, epoch):
        pbar = tqdm(self.vloader, total=len(self.tset), desc="Validation")
        metrics = Metrics(self.tset.cnames)
        self.model.eval()
        with torch.no_grad():
            for i, sample in enumerate(pbar):
        ...
        self.writer.add_image("val_input", tset.to_rgb(x[0]), epoch, dataformats='HWC')
        self.writer.add_image("val_label", tset.color_labels(y[0]), epoch, dataformats='HWC')
        self.writer.add_image("val_pred", tset.color_labels(pred), epoch, dataformats='HWC')

        self.model.train()
        return tmetrics.percent_mIoU()
