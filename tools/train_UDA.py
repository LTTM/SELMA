...
class TrainerUDA(Trainer):
    def __init__(self, args, writer, logger):
        ...
        self.ttset = args.target_dataset(root_path=args.target_root_path,
                                         ...
                                         augment_data=False,#args.augment_data,
        ...
        if hasattr(args, 'ckpt_file') and args.ckpt_file is not None:
            ...
            if not os.path.exists(args.ckpt_file):
                self.logger.info("Checkpoint [%s] not found, retraining from scratch..."%(args.ckpt_file))
            else:
    ...
    def train_uda_epoch(self, epoch):
        ...
        for i, sample in enumerate(pbar):
            ...
            l_ms = self.msiw(out)
            lt = self.args.lambda_msiw*l_ms

            self.writer.add_scalar('msiw', l_ms.item(), curr_iter)
            lt.backward()
