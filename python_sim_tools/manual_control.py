...
from cityscapes_colmap import cityCMAP
from custom_bp_tags import custom_tag, override_parked_vehicles
...
class World(object):
    ...
    def restart(self):
        # override parked vehicles
        override_parked_vehicles(self.world)
        ...
        if self.player is not None:
            ...
            # update tags
            tags_dict = custom_tag(blueprint)
            if tags_dict is not None:
                self.player.update_semantic_tags(tags_dict)
            # done
...
class CameraManager(object):
    ...
    def _parse_image(weak_self, image):
        ...
        elif 'CityScapes' in self.sensors[self.index][2]:
            image.convert(self.sensors[self.index][1])
            array = np.frombuffer(image.raw_data, dtype=np.dtype("uint8"))
            array = np.reshape(array, (image.height, image.width, 4))
            array = array[:, :, 2] # BGRD data -> grayscale label
            color = self.colmap.color(array)
            self.surface = pygame.surfarray.make_surface(color.swapaxes(0, 1))
        else:
            image.convert(self.sensors[self.index][1])
            array = np.frombuffer(image.raw_data, dtype=np.dtype("uint8"))
            array = np.reshape(array, (image.height, image.width, 4))
