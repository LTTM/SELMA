...
class BaseDataset(Dataset):
    ...
    def resize_and_crop(self, rgb=None, gt=None, depth=None):
        if self.resize_to is not None:
            rgb = cv.resize(rgb, self.resize_to, interpolation=cv.INTER_AREA) if rgb is not None # usually images are downsized, best results obtained with inter_area
            gt = cv.resize(gt, self.resize_to, interpolation=cv.INTER_NEAREST_EXACT) if gt is not None # labels must be mapped as-is
            depth = cv.resize(depth, self.resize_to, interpolation=cv.INTER_NEAREST_EXACT) if depth is not None

        if self.crop_size is not None:
            H, W, _ = rgb.shape
            dh, dw = H-self.crop_size[1], H-self.crop_size[0]
            ...
            rgb = (rgb[h0:h0+self.crop_size[1], w0:w0+self.crop_size[0], ...]).copy() if rgb is not None
            gt = (gt[h0:h0+self.crop_size[1], w0:w0+self.crop_size[0], ...]).copy() if gt is not None
            depth = (depth[h0:h0+self.crop_size[1], w0:w0+self.crop_size[0], ...]).copy() if depth is not None

        return rgb, gt, depth

    def data_augment(self, rgb=None, gt=None, depth=None):
        if self.kwargs['flip'] and random.random()<.5:
            rgb = (rgb[:,::-1,...]).copy() if rgb is not None
            gt = (gt[:,::-1,...]).copy() if gt is not None
            depth = (gt[:,::-1,...]).copy() if depth is not None
        if self.kwargs['gaussian_blur']:
            sigma = random.random()*self.kwargs['blur_mul']
            rgb = cv.GaussianBlur(rgb, (0,0), sigma) if rgb is not None
